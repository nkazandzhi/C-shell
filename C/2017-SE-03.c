...
int main(int argc, char* argv[])
{
	...
	int f1_fd,f2_fd,f3_fd;

	if (( f1_fd = open(argv[2],O_RDONLY) ) == -1)
	{
		err(2, "Error: can't open f1.bin");
	}

	if (( f3_fd = open(argv[1],O_RDONLY) ) == -1)
	{
		close(f1_fd);
		err(3, "Error: can't open patch.bin");
	}

	if (( f2_fd = open(argv[3],O_WRONLY|O_CREAT|O_TRUNC, 0644) ) == -1)
	{
		close(f1_fd);
		close(f3_fd);
		err(4, "Error: can't create f3.bin");
	}
	...
	while((bytes_read=read(f1_fd,&c,sizeof(c))) > 0){
		if (write(f2_fd,&c,sizeof(c)) != bytes_read)
		{
			close(f1_fd);
			close(f2_fd);
			close(f3_fd);
			err(5, "Error: cant write");
		}
	}
	...
	if (bytes_read == -1)
	{
		...
		close(f2_fd);
		close(f3_fd);
		err(6, "Error: reading from f1.bin failed");
	}

	struct pos p;
	while(read(f3_fd,&p,sizeof(p)) == sizeof(p))
	{
		off_t l = lseek(f2_fd, p.offset_, SEEK_SET);
		if ( l == -1 )
		{
			close(f1_fd);
			close(f2_fd);
		...
		if ((write(f2_fd,&p.replacement_, sizeof(p.replacement_)) != sizeof(p.replacement_))){
		
			close(f1_fd);
			close(f2_fd);
			close(f3_fd);
			err(7, "Error: writing failed");
		}
